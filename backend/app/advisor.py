...
=======
>>>>>>> theirs
        habit_alerts = [pattern.category for pattern in self.behavior.weekly_patterns(db, period_start, period_end)[:3]]
        if tasks:
            aligned = sorted(tasks, key=lambda item: (-item.priority, item.estimated_hours))[:3]
            focus_areas.extend([f"Keep {task.title} anchored to goal: {task.long_term_goal}." for task in aligned])
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
